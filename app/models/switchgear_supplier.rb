class SwitchgearSupplier < ApplicationRecord
  belongs_to :switchgear
  validates :switchgear, :presence => true
  belongs_to :supplier
  validates :supplier, :presence => true

  validates :switchgear, uniqueness:  { scope: :supplier_id,
  ...
  def self.import(file)
    ...
    begin
      CSV.foreach(file.path, :col_sep => (";"), :encoding => 'iso-8859-1', headers: :first_row, header_converters: :symbol) do |row|
        ...
        if SwitchgearSupplier.where(:switchgear_id => new_record[:switchgear_id],
                                                :supplier_id => new_record[:supplier_id]).any?
          # if this device already exists, only update existing entry
          existing_record = SwitchgearSupplier.where(:switchgear_id => new_record[:switchgear_id],
          ...
          suppliertypeid = Suppliertype.where(name: 'Schaltanlagenbau').first.id
          if SwitchgearSupplier.new(new_record).valid? && SupplierSuppliertype.where(:supplier_id => new_record[:supplier_id], :suppliertype_id => suppliertypeid).any?
      ...
      records_to_save.each do |record|
        SwitchgearSupplier.create! record
      end
